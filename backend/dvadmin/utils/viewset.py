...
class CustomModelViewSet(ModelViewSet, ImportSerializerMixin, ExportSerializerMixin, QueryArgumentsMixin):
    ...
    # 注释编号:django-vue3-admin_viewset005916
    # 功能说明:统一为所有views注册一条后缀为getbyIds的路由，匹配所有懒加载的标准代码
    ...
    def getbyIds(self, request, *args, **kwargs):
        ...
        queryset = cur_model.objects.filter(id__in=idList)  #过滤出来前端传过来的list列表里面的对象
